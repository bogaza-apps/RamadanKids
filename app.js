...
function contrastColor(hex){
  try{
    ...
    const darkText=s.getPropertyValue('--color-text').trim()||'#1a0e00';
    const lightText=s.getPropertyValue('--color-on-primary').trim()||'#ffffff';
    return hexLuminance(hex)>0.35?darkText:lightText;
  }
...
function getProfileTheme(profileId){
  ...
  const saved=localStorage.getItem('lfa_theme_global');
  if(saved)return saved;
  // First-ever visit: respect OS dark/light preference
...
function loadProfileTheme(){
  ...
  _updateThemeColorMeta(t);
}

// Update <meta name="theme-color"> to match active theme background
const _THEME_COLORS={night:'#0d1b3e',day:'#faf5ec',girls:'#fff4f9',boys:'#f0f8f2'};
function _updateThemeColorMeta(t){
  let meta=document.querySelector('meta[name="theme-color"]');
  if(!meta){meta=document.createElement('meta');meta.name='theme-color';document.head.appendChild(meta);}
  meta.content=_THEME_COLORS[t]||_THEME_COLORS.night;
}
...
function renderBadDeeds(p,log){
  ...
  lst.innerHTML=bds.map(bd=>{const preset=STATE.badDeeds.find(b=>b.id===bd.id)||{};return`<div class="bad-deed-item"><span style="font-size:1.1rem">⚠️</span><span style="flex:1;font-size:0.85rem">${preset.name||bd.id}</span><span style="color:var(--color-danger);font-weight:700;font-size:0.85rem">${preset.pts||''} نقطة</span></div>`}).join('');
}
...
function renderProgress(){
  ...
  if(jl){
    jl.innerHTML=JOURNEY.map((j,idx)=>{
      ...
      return`<div class="journey-item${done?' done':''}${cur?' current':''}"><span style="font-size:1.4rem">${j.icon}</span><span style="flex:1;font-weight:700">${j.name}</span><span class="text-muted text-xs">${j.req} نقطة</span>${done?'<span style="color:var(--color-success)">✓</span>':''}</div>`;
    }).join('');
  }
...
function renderQuizPath(p){
  ...
  // Node styles per state — read from CSS tokens for theme-awareness
  const _cs=getComputedStyle(document.documentElement);
  const _success=_cs.getPropertyValue('--color-success').trim()||'#3dd68c';
  const _successDark=_cs.getPropertyValue('--color-success-dark').trim()||'#1e8a4a';
  const _successGlow=_cs.getPropertyValue('--color-success-glow').trim()||'rgba(61,214,140,0.30)';
  const _danger=_cs.getPropertyValue('--color-danger').trim()||'#f26c6c';
  const _dangerDark=_cs.getPropertyValue('--color-danger-dark').trim()||'#b92d2d';
  const _dangerGlow=_cs.getPropertyValue('--color-danger-glow').trim()||'rgba(242,108,108,0.30)';
  const _primary=_cs.getPropertyValue('--color-primary').trim()||'#f5b731';
  const _primaryDeep=_cs.getPropertyValue('--color-primary-deep').trim()||'#c98a00';
  const _focusRing=_cs.getPropertyValue('--color-focus-ring').trim()||'rgba(245,183,49,0.55)';
  const _surfMid=_cs.getPropertyValue('--color-surf-mid').trim()||'rgba(255,255,255,0.11)';
  const _border=_cs.getPropertyValue('--color-border').trim()||'rgba(255,255,255,0.15)';
  const _onSuccess=_cs.getPropertyValue('--color-on-success').trim()||'#ffffff';
  const _onDanger=_cs.getPropertyValue('--color-on-danger').trim()||'#ffffff';
  const _onPrimary=_cs.getPropertyValue('--color-on-primary').trim()||'#1a0e00';
  const nodeStyle={
    done:  {ring:_successDark,bg:`linear-gradient(135deg,${_success},${_successDark})`,   icon:'✅',numColor:_onSuccess, shadow:`0 4px 15px ${_successGlow}`, border:`3px solid ${_successDark}`},
    missed:{ring:_dangerDark, bg:`linear-gradient(135deg,${_danger},${_dangerDark})`,     icon:'❌',numColor:_onDanger,  shadow:`0 4px 12px ${_dangerGlow}`,  border:`3px solid ${_dangerDark}`},
    today: {ring:_primaryDeep,bg:`linear-gradient(135deg,${_primary},${_primaryDeep})`,   icon:'⭐',numColor:_onPrimary, shadow:`0 0 0 5px ${_focusRing},0 4px 20px ${_focusRing}`,anim:'animation:pulse-today 1.8s ease-in-out infinite',border:`3px solid ${_primaryDeep}`},
    locked:{ring:_border,     bg:_surfMid,                                                icon:'🔒',numColor:'var(--color-text-faint)',shadow:'none',border:`3px solid ${_border}`}
  };
  ...
  for(let row=0;row<totalRows;row++){
    ...
    display.forEach((ds,i)=>{
      ...
      if(!isLast){
        ...
        const connStyle=bothDone
          ?`background:linear-gradient(90deg,${_success},${_successDark})`
          :ds.state==='today'
            ?`background:linear-gradient(90deg,${_primary},rgba(255,255,255,.3))`
    ...
    if(row<totalRows-1){
      ...
      const vStyle=vDone?`background:linear-gradient(180deg,${_success},${_successDark})`:'background:rgba(255,255,255,.3)';
  ...
  html+=`<div class="map-legend">
    <div class="map-legend-item"><div class="map-legend-dot" style="background:${_success}"></div>أجاب ✅</div>
    <div class="map-legend-item"><div class="map-legend-dot" style="background:${_danger}"></div>فاته 💔</div>
    <div class="map-legend-item"><div class="map-legend-dot" style="background:${_primary}"></div>اليوم 🌟</div>
    <div class="map-legend-item"><div class="map-legend-dot" style="background:${_surfMid};border:1.5px solid ${_border}"></div>قادم 🔒</div>
  </div>`;
  ...
  sg.innerHTML=cats.map(cat=>{
    ...
    const rows=items.map(item=>{
      ...
      return`<div class="shop-item${owned?' owned':''}">
        ...
        <div style="color:var(--color-secondary);font-weight:900;margin:5px 0;font-size:0.9rem">💎 ${item.price}</div>
...
function renderAllQList(allQ){
  ...
  const diffColor = {
    easy:  cs.getPropertyValue('--color-success').trim()||'#3dd68c',
    medium:cs.getPropertyValue('--color-primary').trim()||'#f5b731',
    hard:  cs.getPropertyValue('--color-danger').trim()||'#f26c6c'
  };
  ...
  list.innerHTML = filtered.slice(0,100).map((q,i)=>{
    ...
    return`<div style="background:var(--surf2);border:1px solid var(--bdr);border-radius:11px;padding:10px 12px" data-qid="${q.id}">
      <div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:6px">
        <span style="font-size:0.68rem;font-weight:900;color:var(--color-on-primary);background:${diffColor[qDiff]};padding:2px 7px;border-radius:8px;flex-shrink:0;margin-top:2px">${diffLabel[qDiff]}</span>
        ...
        ${isCustom?`<span style="font-size:0.65rem;background:var(--color-surf-mid);color:var(--color-primary);padding:2px 7px;border-radius:8px;flex-shrink:0;margin-top:2px">مخصص</span>`:''}
        <div style="flex:1;font-weight:700;font-size:0.88rem;line-height:1.4">${q.q}</div>
      </div>
      <div style="font-size:0.78rem;color:var(--muted);margin-bottom:8px">✅ الإجابة: <span style="color:var(--color-success);font-weight:800">${answerText}</span> &nbsp;|&nbsp; نقاط: ${q.p||10}</div>
...
function renderRewards(){
  ...
  el.innerHTML=[...rws].reverse().map((r,i)=>{
    ...
    const claimed=r.claimed?`<span class="badge" style="background:var(--color-success)">تم الاستلام ✅</span>`:`<span class="badge" style="background:var(--color-primary)">قيد الانتظار ⏳</span>`;
...
function confetti(){
  const _ccs=getComputedStyle(document.documentElement);
  const cols=[
    _ccs.getPropertyValue('--color-primary').trim()||'#f5b731',
    _ccs.getPropertyValue('--color-secondary').trim()||'#34c4ab',
    _ccs.getPropertyValue('--color-danger').trim()||'#f26c6c',
    _ccs.getPropertyValue('--color-tertiary').trim()||'#9b6dff',
    _ccs.getPropertyValue('--color-secondary-dim').trim()||'#7de8d5',
    _ccs.getPropertyValue('--color-success').trim()||'#3dd68c'
  ];
